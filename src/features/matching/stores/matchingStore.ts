...
type MatchingAction =
  ...
  | { type: 'UPDATE_MATCH_STATUS'; payload: { matchId: string; status: MatchStatus } }
    ...
    case 'UPDATE_STUDENT_PROFILE':
      return {
        ...
        currentStudent: state.currentStudent
          ? { ...state.currentStudent, ...action.payload, updated_at: new Date() }
          : null,
      };
    ...
    case 'UPDATE_TEACHER_PROFILE':
      return {
        ...
        currentTeacher: state.currentTeacher
          ? { ...state.currentTeacher, ...action.payload, updated_at: new Date() }
          : null,
      };
      ...
      return {
        ...
        currentMatches: state.currentMatches.map(match =>
          match.match_id === action.payload.matchId
            ? { ...match, status: action.payload.status, updated_at: new Date() }
            : match
        ),
      };

    case 'ADD_COURSE':
      return { ...state, currentCourses: [...state.currentCourses, action.payload] };
...
interface MatchingContextType extends MatchingState {
  ...
  // Matching actions
  findMatches: (student: Student, subject?: string, minScore?: number) => Promise<void>;
...
export function MatchingProvider({ children }: { children: ReactNode }) {
  ...
  const contextValue: MatchingContextType = {
    ...
    // Profile management
    setCurrentStudent: (student) => dispatch({ type: 'SET_CURRENT_STUDENT', payload: student }),
    setCurrentTeacher: (teacher) => dispatch({ type: 'SET_CURRENT_TEACHER', payload: teacher }),

    updateStudentProfile: (updates) => dispatch({ type: 'UPDATE_STUDENT_PROFILE', payload: updates }),
    updateTeacherProfile: (updates) => dispatch({ type: 'UPDATE_TEACHER_PROFILE', payload: updates }),
    ...
    findMatches: async (student, subject, minScore = 60) => {
      ...
      try {
        ...
        const result = MatchingService.findMatches(student, state.availableTeachers, subject, minScore);
        dispatch({ type: 'SET_MATCHES', payload: result.matches });
        dispatch({ type: 'SET_LOADING', payload: false });
      } catch (error) {
        dispatch({
          type: 'SET_ERROR',
          payload: error instanceof Error ? error.message : 'Failed to find matches'
        });
        dispatch({ type: 'SET_LOADING', payload: false });
      }
    },
    ...
    acceptMatch: async (matchId) => {
      ...
      try {
        ...
        dispatch({
          type: 'SET_ERROR',
          payload: error instanceof Error ? error.message : 'Failed to accept match'
        });
        dispatch({ type: 'SET_LOADING', payload: false });
      }
    },
    ...
    rejectMatch: async (matchId) => {
      ...
      try {
        ...
        dispatch({
          type: 'SET_ERROR',
          payload: error instanceof Error ? error.message : 'Failed to reject match'
        });
        dispatch({ type: 'SET_LOADING', payload: false });
      }
    },
    ...
    createCourse: async (match, courseData) => {
      ...
      try {
        const newCourse: Course = {
          ...
          title: courseData.title || `Course: ${match.match_criteria.subject_expertise_score > 50 ? 'Advanced' : 'Beginner'} ${courseData.subject || 'Subject'}`,
          subject: courseData.subject || 'General',
          start_date: courseData.start_date || new Date(),
          end_date: courseData.end_date || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000), // 30 days from now
        ...
        dispatch({
          type: 'SET_ERROR',
          payload: error instanceof Error ? error.message : 'Failed to create course'
        });
        dispatch({ type: 'SET_LOADING', payload: false });
      }
    },
    ...
    completeCourse: async (courseId, rating) => {
      ...
      try {
        ...
        dispatch({
          type: 'SET_ERROR',
          payload: error instanceof Error ? error.message : 'Failed to complete course'
        });
        dispatch({ type: 'SET_LOADING', payload: false });
      }
    },
